...
`ssh mears@hpg.rc.ufl.edu`

Then, follow the prompts to enter your password and Duo two-factor authentication.

```bash
(base) % ssh mears@hpg.rc.ufl.edu
Password: xxxxxxxx
Duo two-factor login for mears@ufl.edu
...
```bash 
[mears@login1 ~]$ srun --pty -p hpg2-compute -n 1 -N 1 -t 90 --mem=2gb /bin/bash
```
...
```bash
[mears@c0711a-s6 data]$ echo "Hello from $(hostname)"
